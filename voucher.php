...
<meta name="theme-color" content="#563d7c">
    ...
    <style>
      ...
      #tabla_voucher_container .dataTables_wrapper{
        width: 100% !important;
      }
      #tabla_voucher{
        width: 100% !important;
      ...
      @media (max-width: 768px){
        #tabla_voucher .dt-truncate{ max-width: 140px; }
      }
...
<script>
  ...
  function show_traer_tabla_voucher(){
    setTimeout(() => {
      traer_tabla_voucher()
    }, 500);
  }
  ...
  (function bindTablaCotizacionAdjustOnTab(){
    function bind(){
      ...
      $(document).on('shown.bs.tab', 'a[data-toggle="tab"][href="#profile"]', function () {
        if ($.fn.DataTable.isDataTable('#tabla_voucher') && typeof dtable !== 'undefined') {
          try {
  ...
  function traer_tabla_voucher() {
    ...
    if (!$.fn.DataTable.isDataTable('#tabla_voucher')) {
      dtable = $("#tabla_voucher").DataTable({
        "autoWidth": false,
        "order": [[0, "desc"]],
        "responsive": {
          details: {
        ...
        "columnDefs": [
          ...
          },
          {
            ...
            render: function (data, type) {
              ...
              return `<span title="${formatMoney(data)}">${formatMoney(data)}</span>`;
            }
          },
          {
            targets: 4,
          ...
          },
          {
            ...
            render: function (data, type, row) {
              ...
              if (type !== 'display') return saldo;

              const clase = saldo > 0 ? 'text-danger font-weight-bold' : 'text-success font-weight-bold';
              return `<span class="${clase}" title="${formatMoney(saldo)}">${formatMoney(saldo)}</span>`;
            }
          },
          {
            "targets": 6,
            ...
            render: function (data, type, row) {
              return `
                <button class="btn btn-link" onclick="event.stopPropagation(); ver_voucher(${row.id})">
